...
def main(args):
    ...
    exp_dir = f"{os.getcwd()}/experiments/hpo/trials/CCN_{args.plasticity}_{args.dataset}/"
    os.makedirs(exp_dir, exist_ok=True)
    
    if 'diehl' in args.dataset.split('-'):
        grf_space = [None]
        norm_space = ["L2", "MMS", "MAX"]
    else:
        grf_space = [10, 15, 20, 25, 30]
        norm_space = ["L2", "SS", "MMS", "MAX"]
    
    space = OrderedDict(
            [
                #('CCN+V_th', hp.choice('CCN+V_th', [-55.0, -60.0, -69.6, -69.7, -69.8, -69.9, -69.99])),
                #('CCN+tau_s', hp.choice('CCN+tau_s', [0.1, 0.3, 0.5, 0.7, 0.9])),
                ('CCN+tau_plus', hp.choice('CCN+tau_plus', [5.0, 10.0, 30.0, 50.0, 70.0, 90.0])),
                ('CCN+tau_minus', hp.choice('CCN+tau_minus', [5.0, 10.0, 30.0, 50.0, 70.0, 90.0])),
                #('CCN+ref_seq_interval', hp.choice('CCN+ref_seq_interval', [3,5,7,9])),
                ('CCN+rate', hp.choice('CCN+rate', [100, 500, 1000, 2000, 3000, 4000, 5000, 6000])),
                #('CCN+mu_plus', hp.choice('CCN+mu_plus', [0.0, 0.25, 0.5])),
                #('CCN+mu_minus', hp.choice('CCN+mu_minus ', [0.0, 0.25, 0.5])),
                ...
                ('PRP+norm', hp.choice('PRP+norm', norm_space)),
                ('PCA+n_components', hp.choice('PCA+n_components', [16, 32, 64, 128]))
            ]
        )
    
    pipeline_component_names = ('PRP', 'PCA', 'GRF', 'CCN')
    modules = ('PRP', PCA, GRF, CorrelationClasswiseNetwork)
