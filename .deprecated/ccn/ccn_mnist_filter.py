...
from fsnn_classifiers.correlation_filtering_classifier import CorrelationFilteringClassifier
...
def main(args):
    ...
    cfg["CorrelationClasswiseNetwork"]["quiet"] = False

    if args.n_estimators == 1:
        cfg["CorrelationClasswiseNetwork"]["max_features"] = 1.0
        cfg["CorrelationClasswiseNetwork"]["max_samples"] = 1.0
    ...
    grf = GRF(**cfg["GRF"])
    net = CorrelationFilteringClassifier(**cfg["CorrelationClasswiseNetwork"])
    ...
    if args.test_time is not None:
        model.named_steps["correlationclasswisenetwork"].ccn_1.init_encoder(args.test_time)
        model.named_steps["correlationclasswisenetwork"].ccn_2.init_encoder(args.test_time)
        model.named_steps["correlationclasswisenetwork"].ccn_3.init_encoder(args.test_time)
...
if __name__ == "__main__":
    ...
    filename = os.path.join(out_path, f"ccn_filter_{args.dataset}_{pl_token}_{par_token}.json")
