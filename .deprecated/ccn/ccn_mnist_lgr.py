...
def main(args):
    ...
    lgr = LogisticRegression(C=1000, max_iter=100000)
    ...
    X_lgr = []
    n_splits = 18
    m = len(X_train) // n_splits 
    for k in range(n_splits):
        x_ = model.transform(X_train[m*k:m*(k+1)])
        X_lgr.append(x_)

    X_train = np.concatenate(X_lgr, axis=0)
    ...
    X_lgr = []
    n_splits = 2
    m = len(X_test) // n_splits
    for k in range(n_splits):
        x_ = model.transform(X_test[m*k:m*(k+1)])
        X_lgr.append(x_)

    X_test = np.concatenate(X_lgr, axis=0)
