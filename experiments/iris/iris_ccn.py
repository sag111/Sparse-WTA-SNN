...
def run_script(script_suffix, plasticity):
    result = subprocess.run(['python', '/s/ls4/users/selibrin/spiking_researches/Sparse-WTA-SNN/experiments/iris/run_ccn_iris_fold.py', str(script_suffix), str(plasticity)], capture_output=True, text=True)
...
def run(args):
    ...
    path = '/s/ls4/users/selibrin/spiking_researches/Sparse-WTA-SNN/experiments/iris/data_for_folds/' + ccn.synapse_model
    ...
    preproccessing = params['norm']

    grf_params = grf.__dict__

    ccn_params = params
    # ccn_params["synapse_model"] = "stdp_gaussian_times_linear_with_separate_exp_r_dependence_synapse"
    ...
    print("\n\n\n")
    print("max rate dec : " + str(all_scores))
    print("\n")
    print("log regr dec : " + str(all_lr_scores))
