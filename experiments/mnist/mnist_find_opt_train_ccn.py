...
def run(args):
    ...
    if args.full_ds:
        ...
        X_train, X_test, y_train, y_test = load_data("mnist", max_train=max_train) 
        ...
        for _ in range(5):
            for i in range(20):
                train_size = int((i + 1) * max_train / 20)

                pipe.fit(X_train[:train_size], y_train[:train_size])

                y_pr = pipe.predict(X_test[:1000])

                score = f1_score(y_test[:1000], y_pr, average='micro')
                print(f"F1-micro: {score}")

                with open(f"{parent_directory}/results/train_size_and_scores_test_1K.pkl", 'ab') as fp:
                    pickle.dump(f"train_size: {train_size}, score={round(score, 2)}", fp)
