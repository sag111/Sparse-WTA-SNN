...
def run_script(neuron, ensemble_number, job_id):
    result = subprocess.run([
        'python',
        '/s/ls4/users/selibrin/spiking_researches/Sparse-WTA-SNN/experiments/mnist/run_one_neuron_of_ccn.py',
        str(neuron), 
        str(ensemble_number),  
        str(job_id)],
        capture_output=True,
...
if __name__ == "__main__":
    ...
    path_to_params = f'/s/ls4/users/selibrin/spiking_researches/Sparse-WTA-SNN/experiments/mnist/data_from_runs/{job_id}/params.pkl'
    ...
    with ThreadPoolExecutor(max_workers=n_classes) as executor:
        futures = [executor.submit(run_script, i, ensemble_number, job_id) for i in range(n_classes)]
